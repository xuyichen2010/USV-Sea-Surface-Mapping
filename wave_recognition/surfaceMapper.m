function surfaceMapper( bag, varargin)
% INTELVIEW is a basic viewer GUI for Intel RealSense stereo cameras
%
%   surfaceMapper( bag )
%   surfaceMapper( bag, msg_number )
%   surfaceMapper( bag, msg_number, volume_of_interest )
%
%   ARGUMENTS:
...
%     view(az,el);
    ...
    subplot(h3);
    plot(depth_view, cut_view,'b','lineWidth',2);
    hold on;
...
%     xlabel('frame number','FontSize',16);
    ...
    if size(minimal1_4) > 0
        [bestParameter1,bestParameter2] = ransac_demo(transpose(minimal1_4),2,100,0.1,0.2);
        xLine = 1:640;
        yLine = bestParameter1*xLine + bestParameter2;
...
%             temp = reshape(covPts(1:realIndex1,:,:), [realIndex1*n,k]);
%             top_rep = nanmean(temp(:,3));
%             temp = reshape(covPts(realIndex2:end,:,:), [(m-realIndex2+1)*n,k]);
%             bot_rep = nanmean(temp(:,3));
            count_col = 0;
            count_col1 = 0;
            for i=1:640
        ...
        el = plot(r_ellipse(:,1) + X0,r_ellipse(:,2) + Y0, 'black', 'lineWidth', 1);
        hold on;
        ...
        if prev_slope ~= 0 
            if abs(slope-prev_slope) / prev_slope > 0.3
                slope = prev_slope;
            end
        end
        prev_slope = slope;
        ...
        co = scatter(covPts(:,1), covPts(:,2),'red');
